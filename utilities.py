...
# File loaders 
#################
def load_gnn_graphs(dataset_name, embed, typ):
    ...
    for graph in graphs:
        # Load features
        node_labels = torch.LongTensor(graph.ndata['node_labels'].numpy())
        node_labels_embed = embed(node_labels)
        node_labels_embed = torch.squeeze(node_labels_embed)
        if graph.ndata.get('node_attr') != None and typ == 'both':
            node_attr = torch.FloatTensor(graph.ndata['node_attr'].numpy().astype(float))
            node_labels_embed = torch.cat((node_labels_embed, node_attr), dim=1)
        graph.ndata['node_embed'] = torch.FloatTensor(node_labels_embed.detach().numpy())
        if graph.edata.get('node_labels') != None:
            graph.edata['node_labels'] = graph.edata['node_labels'].type(torch.FloatTensor)
        else:
            graph.edata['node_labels'] = torch.FloatTensor(np.ones_like(graph.edata['edge_labels']))
    n_node_embed = node_labels_embed.shape[1]

    return data, graphs, n_node_embed
...
def load_continuous_graphs(dataset_name):
    ...
    return node_labels, node_features, adj_mat, n_nodes, edge_features, labels, graphs
